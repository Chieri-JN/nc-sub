...
class Solution:
    def characterReplacement(self, s: str, k: int) -> int:
        ...
        for i in range(n):
            # if j >= 91:
                # print(f"CURR SS s[{j}:{i}+1]: {s[j:i+1]}")
            # print(f"charFreq ---> {charFreq}")
            # print(f"freqChar ---> {freqChar}")
            # print(f"mChar: {mChar}")
            ...
            # if j >= 90:
                # print(f" charFreq[{mChar}] > charFreq[{s[i]}] {charFreq[mChar] > charFreq[s[i]]}")
            mChar = mChar if mChar and charFreq[mChar] > charFreq[s[i]]  else s[i]
            # print(f"k used with mChar of {mChar}: {(i - j) - charFreq[mChar]}")
            # print(f"S (mChar): {charFreq[mChar]}, j:{j}, i:{i}")
            # print(f"{(i - j + 1) - charFreq[mChar]} > {k}")
            # if j >= 90: 
                # print(f"charFreq[S] ---> {charFreq.get("S", 0)}")
                # print(f"S {mChar}: {charFreq[mChar]}, j:{j}, i:{i}")
                # print(f"{(i - j + 1) - charFreq[mChar]} > {k}")
            while (i - j + 1) - charFreq[mChar] > k:
                ...
                count = charFreq[rmC]
                # if j >= 90:
                    # print(f"BEFORE {s[j:i+1]}")
                    # print(f"charFreq[S] ---> {charFreq.get("S", 0)}")
                    # print(f"--- rmC: {rmC}, count: {count}")
                ...
                j += 1
                # if j >= 90:
                    # print(f"AFTER {s[j:i+1]}")
                    # print(f"mChar {mChar}: {charFreq[mChar]}, j:{j}, i:{i}")
                    # print(f"charFreq[S] ---> {charFreq.get("S", 0)}")
            maxSS = max(i - j, maxSS)
